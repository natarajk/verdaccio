...
module.exports = function(config, auth, storage) {
  ...
  app.get('/-/install/:package/:version?', can('access'), function(req, res, next) {
	var sys = require('util')
	var exec = require('child_process').exec;
	function puts(error, stdout, stderr) { 
           if(error) next("ERROR: Error in installing package " + error); 
           next(stdout.replace(/\n/g, "<br />")) 
        }
	exec("./nbinstall/nbinstall_helper " + req.params.package + " " + req.params.version, puts); 
  })
